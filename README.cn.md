...
- 关于如何配置 `~/.ssh/config`，请参考 [openssh](https://manpages.debian.org/bookworm/openssh-client/ssh_config.5.en.html)，或参考 tssh wiki [SSH基本配置](https://github.com/trzsz/trzsz-ssh/wiki/SSH%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE)。

  - 说明：在 `tssh.conf` 中设置 `UseOpenSSHConfig=yes` 时，支持通过 `ssh -G` 计算 OpenSSH 配置。
